...
esp_err_t alpaca_guideratera_put_handler(httpd_req_t *req) {
    ...
    char *val = alpaca_get_form_param(req, "GuideRateRightAscension");
    if (!val) {
        alpaca_response_error(req, 1025, "Missing GuideRateRightAscension", cid, stx);
        return ESP_OK;
    }
    ...
    if (end == val || *end != '\0' || !isfinite(rate)) {
        free(val);
        alpaca_response_error(req, 1025, "Invalid GuideRateRightAscension", cid, stx);
        return ESP_OK;
    }
    free(val);
    ...
    alpaca_bridge_set_guide_rate_ra(rate);
